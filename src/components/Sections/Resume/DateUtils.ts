export const calculatePeriod = (startDate: string, endDate: string = 'Present'): string => {
  ...
  export const extractDatesFromString = (dateString: string): [string, string] => {
    const [startMonth, startYear, endMonth, endYear] = dateString.match(/(January|February|March|April|May|June|July|August|September|October|November|December)\s+(\d{4})(?:\s*-\s*(?:(January|February|March|April|May|June|July|August|September|October|November|December)\s+(\d{4})|Present))?/)?.slice(1) ?? [];
    ...
    const endDate = endMonth ? `${endYear}-${monthToNumber[endMonth]}-${new Date(Number(endYear), monthToNumber[endMonth] as unknown as number, 0).getDate()}` : 'Present';
